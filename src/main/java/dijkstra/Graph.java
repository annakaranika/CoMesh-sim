...
public class Graph {
    ...
    public Graph(Graph graph) {
        ...
        for (Node node: graph.getNodes()) {
            ...
            for (Entry<Node, Float> adjacentNode: node.getAdjacentNodes().entrySet()) {
    ...
    public void print() {
        Network.log("topology.print():");
        for (Node node: nodes) {
            Network.log(node.getID() + ": " + node.getAdjacentNodes());
            Network.log("distance: " + node.getDistance() + ", shortest path: " + node.getShortestPath());
        }
    }
