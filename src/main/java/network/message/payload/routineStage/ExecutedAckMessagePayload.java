...
public class ExecutedAckMessagePayload extends ReplyMessagePayload {
    private static final long serialVersionUID = 4548420034890318714L;

    public ExecutedAckMessagePayload(int epochNo, String rtnID, int srcSeqNo) {
    ...
    public Pair<MessageType, ReplyMessagePayload> process(
        ...
        if (msgInfo == null) {
            mngr.log("Msg seq no " + srcSeqNo + " does not exist!\n\t" + this);
        }
        if (msgInfo != null && !msgInfo.isApproved()) {
            msgInfo.newReply(src, true);
            final int neededAckCount = mngr.getF() + 1;
            mngr.log(kgrp, "Received new executed ack (" + msgInfo.getPositiveRepliesCount()
                    + "/" + neededAckCount + " now)");
            if (msgInfo.getPositiveRepliesCount() >= neededAckCount) {
                String rtnID = getMonitored1();
                ExecutedMessagePayload ogPayload = (ExecutedMessagePayload) msgInfo.getMsgPayload();
                int rtnSeqNo = ogPayload.rtnSeqNo;
                mngr.log("Routine " + rtnID + "-" + rtnSeqNo + " execution replicated");
                mngr.removeMsgInfo(srcSeqNo);
            }
        }
    ...
    public String toString() {
        return "EXECUTED_ACK " + super.toString();
    }
}
